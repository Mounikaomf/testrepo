  ...
  "Statement":[
    {
      ...
      "Condition":{
        "ArnEquals":{
          "aws:SourceArn":"arn:aws:sns:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:${var.account_code}-${var.env}-bnf-s32sns-${var.region_code}"
        }
      }
    }
  ]
...
resource "aws_sns_topic_subscription" "bnf-subscription" {
  topic_arn = "arn:aws:sns:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:${var.account_code}-${var.env}-bnf-s32sns-${var.region_code}"
  protocol  = "sqs"

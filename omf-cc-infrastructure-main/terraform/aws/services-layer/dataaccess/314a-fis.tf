module "module314a-fis-sqs2sftp" {
  source = "../../modules/sqs2sftp"

  account_code = var.account_code
  env = var.env
  feedname = "Report314a-fis"
  name = "314a-fis"
  ...
  snstopic_arn = "arn:aws:sns:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:${var.account_code}-acc-sfexchange-314a-fis-${var.region_code}"
  ...
  source_filter_prefix = "314a-fis/out"
